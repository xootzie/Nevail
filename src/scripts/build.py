...
def main():
    ...
    with open(appinfo["logoSvgFilePath"], "rb") as svg_file:
        svg_data = svg_file.read()

    for path, size in pngIcons.items():
        svg2png(bytestring=svg_data, write_to=path, output_height=size, output_width=size)
    ...
    for path, size in icnsIcons.items():
        svg2png(bytestring=svg_data, write_to="temp.png", output_height=size, output_width=size)
    ...
    placeholder_files = [
        ...
        "src/packages/debian/changelog.in",
        ...
        "src/scripts/build/package-linux-tarball",
        "src/scripts/build/package-flatpak",

        "src/packages/linux-tarball/install.sh",
        "src/packages/linux-tarball/uninstall.sh",
        "src/packages/linux-tarball/NEUTRON_INTERNAL_APP_NAME.desktop",
    ]
    ...
    for platform in appinfo["platforms"]:
        ...
        elif platform == "linux-tarball":
            buildable_platforms.append("linux")
            buildable_platforms.append("launch-app-linux")
            buildable_platforms.append("package-linux-tarball")
        elif platform == "flatpak":
            buildable_platforms.append("linux")
            buildable_platforms.append("launch-app-linux")
            buildable_platforms.append("package-flatpak")
        else:
